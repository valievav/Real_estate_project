...
2. **Use Puttygen to create key pair**
...
1. **Enter ssh key into Digital Ocean Settings/Security** 
...
1. **Setup Putty to connect to server via local machine** - use [instructions](https://www.digitalocean.com/docs/droplets/how-to/connect-with-ssh/putty/)
...
User=root
Group=www-data
WorkingDirectory=/root/pyapps/Real_estate_project
ExecStart=/root/pyapps/venv/bin/gunicorn \
          --access-logfile - \
          --workers 3 \
...
server {
    ...
    location /static/ {
        root /root/pyapps/Real_estate_project;
    }
...
6. **Increase the max upload size in nginx conf** - sudo nano /etc/nginx/nginx.conf
7. **Restart NGINX** - sudo systemctl restart nginx
8. **GOTO ip (w/o port - automatically running on 80)** - website up and running :)


#TODO - convert ssh key to putty, use putty
